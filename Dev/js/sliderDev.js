...
class Slider {
    ...
    constructor(sliderSelector, settings) {
        ...
        this.nextClass = `${sliderSelector.slice(1)}__next`;
        this.prevClass = `${sliderSelector.slice(1)}__prev`;
    ...
    updateCurrentSlideSpan() {
        let curSlide = this.currentSlide + 1;
        document.querySelector(`${this.sliderSelector}__current-slide-span`).textContent = `Показываемый слайд: ${curSlide}`;
    }
    ...
    beforeInit() {
        ...
        const addListeners = (() => {
            ...
            if (this.arrowsEnable) {
                ...
                this.sliderWrapper.after(next);
                this.sliderWrapper.after(prev);
